...
# Hide the below details in .env file
SECRET_KEY = "asdfljh23442390ue9rgow39iu90u30948rr324lhefoifjf0e9rru349r0"
ALGORITHM = "HS256"
ACCESS_TOKEN_EXPIERS_MINUTES = 30
...
def create_access_token(email:str, user_id:UUID, expires_delete:timedelta) -> str:
    encode = {
        'sub': email,
        'id': str(user_id),
        'exp': datetime.now(timezone.utc) + expires_delete
    }
...
def login_for_access_token(form_data:Annotated[OAuth2PasswordRequestForm, Depends()], db:Session) -> model.Token:
    ...
    # if (user != False):
    #     token = create_access_token(user.email, user.id, timedelta(minutes=ACCESS_TOKEN_EXPIERS_MINUTES))
